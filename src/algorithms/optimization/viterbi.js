...
/**
 * A nested map such that two state names in order gives the probability of a transition from the first to the second:
 ...
 */
/**
 * A nested map such that a state name followed by an observation name gives the probability of that observation resulting from that state:
 ...
 * @param  {StateSpace} S
 * @param  {Object<String, Number>} P0 - a map which gives the probability that each state in S is the initial hidden state
...
function viterbi(O, S, P0, Y, A, B) {
  /** probability of the state with greatest likelihood at each observation, given the previous state
   *
   * the innermost (nested) arrays give probabilities at each observation in sequence; each member corresponds with a successive observation
   * the outermost (container) array enumerates all possible probability sequences; each member assumes the unique initial state at the same index in S
   * @type Number[][]
   */
  const T1 = [];

  /** state (with corresponding probability in T1) with greatest likelihood at each observation, given the previous state
   * @type State[][]
   */
  const T2 = [];
  ...
  for (let j = 1; j < Y.length; ++j) { //  for each observation (in sequence)
    for (let i = 0; i < S.length; ++i) { //  find the probability of every possible state
      let Pmax = -1; //  guarantee inner conditional satisfied on first iteration
      let kPmax;
      let k = 0;
      do {
        let p = T1[k][j - 1] * A[ S[k] ][ S[i] ] * B[ S[i] ][ Y[j] ];
        if (p > Pmax) {
          Pmax = p;
          kPmax = k;
        }
  ...
  X[T - 1] = S[Z[T - 1]];
  for (let i = 1; i < S.length; ++i) { // skip the value used to init Z[T - 1]
